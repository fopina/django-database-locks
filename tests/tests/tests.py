...
@override_settings(DB_LOCKS_ENABLED=True)
class Test(TestCase):
    def setUp(self):
        self._thread_patch = mock.patch('db_locks.locks.RenewThread.start')
        self._thread_mock = self._thread_patch.start()
        self._lock_patch = mock.patch('db_locks.locks.DBLock')
    ...
    def test_lock_not_supported(self, feat_mock):
        ...
        with self.assertLogs(logger='db_locks', level='ERROR') as cm:
            with db_locks.lock('not_locked', timeout=2):
                self._lock_mock.assert_not_called()
        self.assertEqual(
            cm.output,
            [
                'ERROR:db_locks.locks:db_locks cannot be used with the current database engine as it does not '
                'support SELECT .. FOR UPDATE, proceed at your own risk'
            ],
        )

    def test_lock(self):
        with self.assertLogs(logger='db_locks', level='INFO') as cm:
            with db_locks.lock('testing'):
        ...
        self.assertEqual(cm.output, ['INFO:db_locks.locks:acquiring lock testing'])
    ...
    def test_lock_fail_but_no_retry(self):
        ...
        with self.assertRaisesMessage(
            db_locks.locks.LockException, 'failed to acquire lock'
        ):
            with db_locks.lock('testing', retry=False):
                pass
        self._lock_mock.assert_called_once_with('testing', locked_by=None)
    ...
    def test_lock_decorator_with_parameters(self):
        @db_locks.locked('decorator_test', timeout=2, auto_renew=False, lock_ttl=1)
        def locked_func():
    ...
    def test_status_file(self):
        ...
        with override_settings(DB_LOCKS_STATUS_FILE=f):
            db_locks.locks._status_file('1')
        with open(f) as fd:
            self.assertEqual(fd.read(), '1')

        # feed a directory for error
        f = tempfile.mkdtemp()
        with self.assertLogs('db_locks', level='ERROR') as log:
            with override_settings(DB_LOCKS_STATUS_FILE=f):
                db_locks.locks._status_file('1')
        self.assertIn(
            'ERROR:db_locks.locks:failed to update lock status file', log.output[0]
        )
...
class TestRenewThread(TestCase):
    ...
    @mock.patch('db_locks.locks.DBLock')
    def test_renew_thread(self, lock_mock):
        ml = lock_mock.return_value
        with mock.patch('db_locks.locks.RenewThread') as rt_mock:
            with db_locks.lock('testing'):
        ...
        rt = db_locks.locks.RenewThread(ml, 10)
    ...
    def test_renew_thread_fail(self):
        ...
        self.assertEqual(
            log.output, ['ERROR:db_locks.locks:failed to re-acquire lock test_lock']
        )
    ...
    def test_renew_thread_exception(self):
        ...
        kill_mock.assert_called_once()
        self.assertIn(
            'ERROR:db_locks.locks:some error re-acquiring lock test_lock',
            log.output[0],
        )
...
class TestLock(TestCase):
    def test_multiple_locks(self):
        l1 = db_locks.locks.DBLock('x1')
        l2 = db_locks.locks.DBLock('x2')
        with self.assertLogs('db_locks', level='DEBUG') as logs:
            self.assertTrue(l1.acquire())
        self.assertEqual(
            logs.output,
            [
                'DEBUG:db_locks.locks:lock x1 not yet created, trying to create (and acquire)',
                'DEBUG:db_locks.locks:lock x1 (created and) acquired',
            ],
        )
        ...
        self.assertEqual(
            logs.output,
            [
                'DEBUG:db_locks.locks:lock x2 not yet created, trying to create (and acquire)',
                'DEBUG:db_locks.locks:lock x2 (created and) acquired',
            ],
        )

    def test_locked_lock_same_owner(self):
        l1 = db_locks.locks.DBLock('x1')
        l2 = db_locks.locks.DBLock('x1')
        with self.assertLogs('db_locks', level='DEBUG') as logs:
            self.assertTrue(l1.acquire())
        self.assertEqual(
            logs.output,
        ...
        with self.assertLogs('db_locks', level='DEBUG') as logs:
            self.assertTrue(l2.acquire())
        self.assertEqual(logs.output, ['DEBUG:db_locks.locks:lock x1 acquired/renewed'])

    def test_locked_lock_diff_owner(self):
        l1 = db_locks.locks.DBLock('x1', locked_by='dibs')
        l2 = db_locks.locks.DBLock('x1')
        with self.assertLogs('db_locks', level='DEBUG') as logs:
            self.assertTrue(l1.acquire())
        self.assertEqual(
        ...
        self.assertEqual(
            logs.output,
            ['DEBUG:db_locks.locks:lock x1 active and owned by dibs, try later'],
        )
    ...
    def test_locked_lock_diff_owner_with_release(self):
        ...
        self.assertEqual(
            ...
            [
                'DEBUG:db_locks.locks:lock x1 not yet created, trying to create (and acquire)',
                'DEBUG:db_locks.locks:lock x1 (created and) acquired',
                'DEBUG:db_locks.locks:releasing lock x1',
                'DEBUG:db_locks.locks:released lock x1',
            ],
        )
